...
- New `--cwd` flag scans the current directory (or an explicit path) for repos for that run only, without touching the configured `root_dirs`. It is shorthand for `--root` when you are already standing where you want to scan: `gitpane --cwd` uses the current directory, and it wins over `--root` when both are given. Like `--root`, nothing is written back to `config.toml`.
